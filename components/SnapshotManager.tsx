...
const SnapshotManager: React.FC<SnapshotManagerProps> = ({ snapshots, onClose, onRestore, onSave, onDelete }) => {
  ...
  const handleSave = () => {
    onSave(newSnapshotName.trim() || `نسخه ${new Date().toLocaleTimeString('fa-IR')}`);
    setNewSnapshotName('');
  };
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between p-6 border-b border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="font-bold text-xl text-white font-vazir">تاریخچه پروژه</h2>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-8 custom-scrollbar">
          ...
          <section className="bg-zinc-800/30 p-4 rounded-2xl border border-zinc-800">
            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 font-vazir">ثبت نسخه جدید</h3>
            <div className="flex gap-3">
              <input 
                value={newSnapshotName}
                onChange={(e) => setNewSnapshotName(e.target.value)}
                placeholder="نام نسخه (مثلاً: قبل از تغییر استایل)"
                className="flex-1 bg-zinc-950 border border-zinc-800 p-3 rounded-xl text-sm focus:border-blue-500 outline-none transition-all text-white font-vazir"
              />
              <button 
                onClick={handleSave}
              ...
              >
                <Save size={18} /> <span className="font-vazir">ثبت اسنپ‌شات</span>
              </button>
            </div>
          </section>
          ...
          {/* Snapshot List */}
          <section>
            <h3 className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-4 font-vazir">نسخه‌های ذخیره شده</h3>
            {snapshots.length === 0 ? (
              <div className="text-center py-12 border-2 border-dashed border-zinc-800 rounded-3xl opacity-40">
                ...
                <p className="text-sm font-vazir text-zinc-500">هیچ نسخه‌ای یافت نشد.</p>
              </div>
            ) : (
                ...
                {[...snapshots].sort((a, b) => b.timestamp - a.timestamp).map((snap) => (
                  <div key={snap.id} className="group bg-zinc-800/20 border border-zinc-800/50 p-4 rounded-2xl flex items-center justify-between hover:bg-zinc-800/40 transition-all">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <div className="font-bold text-zinc-100 font-vazir text-sm">{snap.name}</div>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-[10px] text-zinc-500 font-vazir flex items-center gap-1">
